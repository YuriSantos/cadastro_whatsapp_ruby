class CadastrosController < ApplicationController
  ...
  def index
    ...
    @cadastros = if search
                   Cadastro.reindex
                   Cadastro.search(search, page: params[:page], per_page: 10)
                 else
                   Cadastro.all.page(params[:page]).per(20)
                   @cadastros = Cadastro.order(nome: :asc)
  ...
  private
    ...
    def cadastro_params
      params.require(:cadastro).permit(:nome, :parte_id, :telefone, :cpf, :data_cadastro, :local, :user_id, :visualizado)
    end
end
